...
# BTB Sizes
declare btb_size=256

# Gshare bit size
declare gshare_size=8
...
for i in ${g_hist_sizes[@]}; do
  ...
  for k in ${ahrt_sizes[@]}; do
    for j in ${trace_list[@]}; do
      ...
      $predictor_path/predictor --trace-file=$trace_file_directory/$j --rat-size=$rat_size --g-history-size=$i --ahrt-size=$k --ahrt-asso=$ahrt_asso --btb-size=$btb_size --gshare-size=$gshare_size >> $results_folder/trace_analysis_${i}_${k}_${j}.txt &
    done
  done
