...
// Table 5 Size (Number of entries)
#define BTB_TBL5_SIZE	(128)
...
// The Fully associative table implemented as a list (Calls only)
static std::list<BranchTBuf> fasso_tbl3;
 ...
 */
	...
	if(br)
	{
		...
		if(br->is_call)
		{
			...
			uint32 address = get_btb_target(br->instruction_addr, INST_CALL);
			if(address != 0)
		...
		{
			uint32 address = get_btb_target(br->instruction_addr, INST_CONDITIONAL);
			if(address != 0)
				(*predicted_target_address) = address;
			return true;
		}
...
/* -------------BRANCH PREDICTION PART ENDS ------------------------------ */
	...
	if(prediction)
	{
		uint32 address = get_btb_target(br->instruction_addr, INST_TYPE_NO_CARE);
		if(address != 0)
			(*predicted_target_address) = address;
	}
...
#endif
{
	...
	if(br)
	{
		...
		if(br->is_call)
		{
			...
			write_btb(br->instruction_addr, actual_target_address,INST_CALL);
			return;
		}

		if(!br->is_conditional)
		{
			write_btb(br->instruction_addr, actual_target_address,INST_CONDITIONAL);
			return;
		}
	}
	...
	if(br)
	{
		...
		write_btb(br->instruction_addr, actual_target_address,INST_TYPE_NO_CARE);
	}
...
** Return Values:       None
**==========================================================================*/
void PREDICTOR::write_btb(const uint32 pc, const uint32 target, INST_TYPE type)
{
	...
	if(type != INST_CALL)
	{
		...
		else
		{
			...
			if(!is_full(fasso_tbl,BTB_TBL3_SIZE))
				...
				insert_item(pc,fasso_tbl2,target,(uint32)BTB_TBL4_SIZE);
	...
	{
		insert_item(pc,fasso_tbl3,target, (uint32)BTB_TBL5_SIZE);
	}
}
...
** Return Values:       None
**==========================================================================*/
uint32 PREDICTOR::get_btb_target(const uint32 address, INST_TYPE type)
{
	uint32 target = 0;
	...
	{
		target = find_item(address,fasso_tbl3);
	}
