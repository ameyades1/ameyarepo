...
void PREDICTOR::write_btb(const uint32 pc, const uint32 target)
{
	...
	else
	{
		...
		insert_item(pc,fasso_tbl,target);
	}
}
...
uint32 PREDICTOR::get_btb_target(const uint32 address)
{
	...
	{
		/*
		 * IF WE LAND HERE, WE CHECK THE FULLY ASSOCIATIVE TABLE
		 */
...
/*
 * Inserts an item in an fully associative table
 */
void PREDICTOR::insert_item(const uint32 pc, std::list<BranchTBuf>& buf, uint32 data)
{
	...
	else if(buf.size() >= BTB_TBL3_SIZE)
	{
		buf.pop_back();
		buf.push_front(item);
	}
