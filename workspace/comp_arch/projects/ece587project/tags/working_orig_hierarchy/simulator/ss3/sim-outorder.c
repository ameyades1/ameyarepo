...
if (cache_il1v)
    {
	/* access victim cache */
	fprintf(stderr,"\n Instruction Victim Cache Check");
	victim_access = 1;
	lat = cache_access(cache_il1v, cmd, baddr, NULL, bsize, /* now */now, /*pudata */NULL, /* repl addr */NULL);

	md_addr_t set = CACHE_SET(cache_il1v, baddr & cache_il1v->set_mask);
	fprintf(stderr,"\nVictim set No = %d\n", set);
	if(0 == victim_access)
	{

		cache_swap(blk,cache_il1v->sets[set].way_head,bsize);
	}
