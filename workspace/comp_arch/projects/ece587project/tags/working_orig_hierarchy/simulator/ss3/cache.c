...
/* Declare victim hit extern: from sim-outorder.c */
extern int victim_access;
...
/* CACHE SWAP FUNCTION
 * performs cache line swap between L1 Instruction or Data cache and corresponding victim cache
 * To be compiled
 * 05/31/2013 - 19.55
 */
void cache_swap (struct cache_blk_t *blk1,
		  struct cache_blk_t *blk2,
		  int bsize
		  )
{
	fprintf(stderr,"\nCache Swap called\n");
	...
	//fprintf(stderr,"\nSize of blk1 data is ... %d\n",sizeof(blk1->data));
	//fprintf(stderr,"\nBsize is %d\n", bsize);
#if 0
	byte_t* ptemp_data = (byte_t*) malloc(sizeof(byte_t) * bsize);

	memcpy((byte_t*)ptemp_data,(byte_t*)blk1->data, bsize);
	memcpy((byte_t*)blk1->data, (byte_t*)blk2->data, bsize);
	memcpy((byte_t*)blk2->data, (byte_t*)ptemp_data, bsize);
