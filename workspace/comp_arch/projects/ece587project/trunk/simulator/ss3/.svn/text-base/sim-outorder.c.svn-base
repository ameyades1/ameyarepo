 ...
 */
/*
 * PREDICTION CACHE START
 */

#define SIZE_MISS_HISTORY_TBL	(10)
struct miss_history_node
{
	md_addr_t addr;
	struct miss_history_node* next;
};

static struct miss_history_node* phead = NULL;
static struct miss_history_node* ptail = NULL;
static struct miss_history_node* pcurrent = NULL;
static void miss_history_init(unsigned int size)
{
	unsigned int index = 0;

	if(NULL == ptail)
	{
		ptail = (struct miss_history_node*)malloc(sizeof(struct miss_history_node));
		ptail->next = NULL;
		phead = ptail;
	}
	...
	for(index = 1; index < size; index++)
	{
		if(ptail)
		{
			ptail->next = (struct miss_history_node*)malloc(sizeof(struct miss_history_node));
			ptail = ptail->next;
		}
	}
	ptail->next = NULL;
...
static void miss_history_add(md_addr_t addr)
{
	// Store old copy of head
	struct miss_history_node* ptemp = phead;

	if(phead && ptail && pcurrent)
	{
		pcurrent->addr = addr;
...
static int miss_history_probe(md_addr_t addr)
{
	struct miss_history_node* ptemp = phead;
	while(ptemp != NULL)
	{
...
static struct stream_buffer *stream_buf = NULL;

static void flush_stream_buffer(struct stream_buffer* sbuf, int sbuf_size)
{
	if(sbuf)
	{
		int i = 0;
...
static int probe_stream_buffer(struct stream_buffer* sbuf, int baddr)
{
	if(sbuf)
	{
		if(sbuf[0].valid)
		{
			if(sbuf[0].baddr == baddr)
				return 1;
		}
	}
	return 0;
...
static void pop_stream_buffer(struct stream_buffer* sbuf, int bsize, int sbuf_size)
{
	if(sbuf)
	{
		int i = 0;
		// Pop the first element copy rest above
		for(i = 1; i < sbuf_size; i++)
		{
			sbuf[i - 1] = sbuf[i];
		}

		// Prefetch the last one
		sbuf[i].valid = 1;
		sbuf[i].baddr = sbuf[i - 1].baddr + bsize;
	}
}

static void prefetch_stream_buffer(struct stream_buffer* sbuf, int baddr, int bsize,int sbuf_size)
{
	if(sbuf)
	{
		int i = 0;
		...
		{
			sbuf[i].valid = 1;
			sbuf[i].baddr = baddr + bsize*i;
		}
	}
}

static struct stream_buffer* stream_buffer_init(int size)
{
	struct stream_buffer* sbuf;
	sbuf = (struct stream_buffer*)malloc(sizeof(struct stream_buffer));
	return sbuf;
}
...
static int bimod_config[1] =
  { /* bimod tbl size */16384 };

/* 2-level predictor config (<l1size> <l2size> <hist_size> <xor>) */
static int twolev_nelt = 4;
static int twolev_config[4] =
  { /* l1size */1, /* l2size */65536, /* hist */16, /* xor */TRUE};

/* combining predictor config (<meta_table_size> */
static int comb_nelt = 1;
static int comb_config[1] =
  { /* meta_table_size */65536 };

/* return address stack (RAS) size */
static int ras_size = 32;
...
static int RUU_size = 256;

/* load/store queue (LSQ) size */
static int LSQ_size = 128;

/* miss history prediction cache size */
static int mpred_cache_size = 0;
...
/* l1 instruction victim cache config, i.e., {<config>|il1v|none} */
static char *cache_il1v_opt;
...
static int mem_lat[2] =
  { /* lat to first chunk */400, /* lat between remaining chunks */20 };
...
/* L1 Data Victim Cache parameters */
#define L1DV_ASSOC				1	// Associativity

/* L1 Instruction Victim Cache parameters */
#define L1IV_ASSOC				1	// Associativity
...
/* level 1 instruction victim cache */
static struct cache_t *cache_il1v;

/* level 1 data cache, entry level data cache */
static struct cache_t *cache_dl1;

/* level 1 data victim cache */
static struct cache_t *cache_dl1v;
...
/* Updated the dl1 access function to check victim cache before dl2 cache or main memory access */
...
/* 05/31/2013 - 19.30 */  
  ...
  if (cache_dl1v)
	...
	/*access victim cache */
      victim_access = 1;
#ifdef __debug__
	fprintf(stderr,"\n Data Victim Cache Check");
#endif
	lat = cache_access(cache_dl1v,cmd,baddr, NULL, bsize, /* now */now, /*pudata */NULL, /* repl addr */NULL);

      if(0 == victim_access)
	  {
    	  cache_swap(blk,cache_dl1v->last_blk,bsize);
	  }
      if (cmd == Read)
    	  return lat;
      else
...
/* Updated the il1 access function to check victim cache before il2 cache or main memory access */
/* Compilation Successful */
/* 05/31/2013 - 19.30 */  
#ifdef __debug__
fprintf(stderr,"\n L1 cache Miss. Entered L1 miss handler function");
fprintf(stderr,"\nLast Replaced Address = %d",cache_il1->last_repl_addr);
#endif
md_addr_t set_miss_hist = CACHE_SET(cache_il1, baddr);
int in_hist_table = 0;

if(mpred_cache_size)
{
	if(miss_history_probe(set_miss_hist))
	{
		in_hist_table = 1;
	}
	else
	{
		miss_history_add(set_miss_hist);
	}
}
...
if (cache_il1v)
	...
	if(cache_probe(cache_il1v,baddr))
	{
		md_addr_t set = CACHE_SET(cache_il1v, baddr);

#ifdef __debug__
		fprintf(stderr,"\nVictim hit set No = %d\n Victim hit address %d\n", set, baddr);
#endif
		cache_flush_addr(cache_il1v,baddr,now);
		cache_access(cache_il1v,Read,cache_il1->last_repl_addr,NULL,bsize,now,NULL,NULL);
	...
	{
#ifdef __debug__
		fprintf(stderr,"\nVictim Miss address = %d\n", baddr);
#endif
		cache_il1v->misses++;
		if(cache_il1->last_repl_addr)
		{
			if(mpred_cache_size)
			{
				if(in_hist_table)
				{
					lat = cache_access(cache_il1v, Read, cache_il1->last_repl_addr, NULL, bsize, /* now */now, /*pudata */NULL, /* repl addr */NULL);
				}
			}
			else
			{
		...
		else
		{
			if(mpred_cache_size)
			{
				if(in_hist_table)
				{
					lat = cache_access(cache_il1v, Read, baddr, NULL, bsize, /* now */now, /*pudata */NULL, /* repl addr */NULL);
				}
			}
			else
			{
...
if(stream_buf)
{
	if(probe_stream_buffer(stream_buf,baddr))
	{
		// Hit
		cache_il1->misses--;
		cache_il1->hits++;
		pop_stream_buffer(stream_buf,bsize, stream_buffer_size);
		return 0;
	}
	else
	{
		flush_stream_buffer(stream_buf, stream_buffer_size);
		prefetch_stream_buffer(stream_buf,baddr + bsize,bsize,stream_buffer_size);
	}
}
...
/* l1 victim inst cache l1 block miss handler function */
static unsigned int			/* latency of block access */
il1v_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
	      md_addr_t baddr,		/* block address to access */
	      int bsize,		/* size of block to access */
	      struct cache_blk_t *blk,	/* ptr to block in upper level */
	      tick_t now)		/* time of access */
...
return 0;
#if 0
fprintf(stderr,"\n L1 Victim cache Miss. Entered L1 Victim miss handler function");
if (cache_il2)
    {
      /* access next level of inst cache hierarchy */
      lat = cache_access(cache_il2, cmd, baddr, NULL, bsize,
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_int(odb, "-pipe:length",
	      "Pipeline length",
	      &pipeline_length, /* default */20,
	      /* print */TRUE, /* format */NULL);
  ...
  opt_reg_int(odb, "-lsq:size",
	      "load/store queue (LSQ) size",
	      &LSQ_size, /* default */128,
	      /* print */TRUE, /* format */NULL);

  /* miss prediction cache */
  opt_reg_int(odb, "-mpredcache:size",
 	      "miss history prediction cache size",
 	      &mpred_cache_size, /* default */0,
 	      /* print */TRUE, /* format */NULL);

  /* stream buffer size */
  opt_reg_int(odb, "-streambuf:size",
   	      "stream buffer size",
   	      &stream_buffer_size, /* default */0,
   	      /* print */TRUE, /* format */NULL);
...
"                -dtlb dtlb:128:4096:32:r\n"
  ...
  opt_reg_int(odb, "-cache:dl1lat",
	      "l1 data cache hit latency (in cycles)",
	      &cache_dl1_lat, /* default */2,
	      /* print */TRUE, /* format */NULL);

  opt_reg_string(odb, "-cache:dl2",
		 "l2 data cache config, i.e., {<config>|none}",
		 &cache_dl2_opt, "ul2:2048:64:8:l",
		 /* print */TRUE, NULL);

  opt_reg_int(odb, "-cache:dl2lat",
	      "l2 data cache hit latency (in cycles)",
	      &cache_dl2_lat, /* default */20,
	      /* print */TRUE, /* format */NULL);

  opt_reg_string(odb, "-cache:dl1v",
  		 "l1 data victim cache config, i.e., {<config>|dl1v|none}",
  		 &cache_dl1v_opt, "NoValue",
  		 /* print */TRUE, NULL);

  opt_reg_note(odb,
		  "This is the L1 Data Victim Cache\n");
...
"      -cache:dl1 ul1:256:32:1:l -cache:dl2 ul2:1024:64:2:l\n"
  ...
  opt_reg_int(odb, "-cache:il2lat",
	      "l2 instruction cache hit latency (in cycles)",
	      &cache_il2_lat, /* default */20,
	      /* print */TRUE, /* format */NULL);

  opt_reg_string(odb, "-cache:il1v",
  		 "l1 instruction victim cache config, i.e., {<config>|il1v|none}",
  		 &cache_il1v_opt, "NoValue",
  		 /* print */TRUE, NULL);

  opt_reg_note(odb,
		  "This is the L1 Instruction Victim Cache\n");
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
    {
      ...
      /* the level 1 instruction victim cache cannot be defined */
	   if(strcmp(cache_dl1v_opt, "none"))
	 fatal("the l1 data cache must defined if the l1 data victim cache is defined");
	   cache_dl1v = NULL;
	...
	fatal("bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
      ...
      // Preserve the L1 I cache parameters
	  int bsize_dl1 = bsize;

	   // Arguments check for L1 Data Victim cache
	   if (sscanf(cache_dl1v_opt, "%[^:]:%d:%d:%d:%c",
			 name, &nsets, &bsize, &assoc, &c) != 5)
	   {
#ifdef __debug__
		   fprintf(stderr,"\nL1 D Victim cache not created!: bad l1 D-victim cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>\n");
#endif
	   }
	   else
	   {

		   // Create the L1 Data Victim cache with same params as L1 D cache
		   // TODO: Update dl1_access_fn, cache_dl1_lat
		   cache_dl1v = cache_create(name, nsets, bsize_dl1, /* balloc */FALSE,
					   /* usize */0, assoc, cache_char2policy(c),
					   dl1_access_fn, /* hit lat */cache_dl1_lat);

#ifdef __debug__
		  fprintf(stderr,"\nL1 D victim cache options: %s\n",cache_dl1v_opt);
#endif
  ...
  if (!mystricmp(cache_il1_opt, "none"))
	...
	fatal("the l1 inst cache must defined if the l2 cache is defined");
      ...
      /* the level 1 instruction victim cache cannot be defined */
      if(strcmp(cache_il1v_opt, "none"))
    fatal("the l1 inst cache must defined if the l1 instruction victim cache is defined");
      cache_il1v = NULL;
    }
	...
	fatal("bad l1 I-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
     ...
     int bsize_il1 = bsize;
      ...
      // Arguments check for L1 Instruction Victim cache
      if (sscanf(cache_il1v_opt, "%[^:]:%d:%d:%d:%c",
      		 name, &nsets, &bsize, &assoc, &c) != 5)
      {
    	  fprintf(stderr,"\nL1 I Victim cache not created!: bad l1 I-victim cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>\n");
      }
      else
      {

		  // Create the L1 Instruction Victim cache with same params as L1 I cache
		  // TODO: Update il1_access_fn, cache_il1_lat
		  cache_il1v = cache_create(name, nsets, bsize_il1, /* balloc */FALSE,
					   /* usize */0, assoc, cache_char2policy(c),
					   il1v_access_fn, /* hit lat */cache_il1_lat);

		  /* create Miss Prediction cache */
		  if(mpred_cache_size)
		    miss_history_init(mpred_cache_size);

#ifdef __debug__
		  fprintf(stderr,"\nL1 I victim cache options: %s\n",cache_il1v_opt);
#endif
      }

      /* create stream buffer if defined */
      if(stream_buffer_size)
      {
    	  stream_buf = stream_buffer_init(stream_buffer_size);
      }
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  if(cache_il1v)
  	  cache_reg_stats(cache_il1v, sdb);
  if (cache_il2
...
ruu_commit(void)
{
  ...
  while (RUU_num > 0 && committed < ruu_commit_width)
      ...
      if (verbose)
	{
      ...
      sim_ret_insn++;
      if ((sim_ret_insn % 1000000) == 0) {
	fprintf(stderr, "Retired %d million instructions in %d cycles\n", (int)sim_ret_insn/1000000, (int)sim_cycle);
      }
  ...
  for (n_issued=0;
      ...
      if (RSLINK_VALID(node))
	{
	  ...
	  if (rs->in_LSQ
	    ...
	    {
	      if (sim_cycle >= rs->decode_cycle + pipeline_length) {
		...
		panic("mis-predicted store");
	      ...
	      n_issued++;
		      ...
		      // Not past the pipeline. Put back in ready queue.
		      readyq_enqueue(rs);
	      }
	    }
	  else
	      ...
	      if (MD_OP_FUCLASS(rs->op) != NA)
		{
		  ...
		  if (fu && (sim_cycle >= rs->decode_cycle + pipeline_length))
		    {
...
#include "machine.def"
	  ...
	  if (MD_OP_FLAGS(op) & F_MEM)
		...
		is_write = TRUE;
          ...
          if (icount && ((icount % 1000000) == 0)) {
                fprintf(stderr, "Fast forward %d million instructions\n",
                                (int)icount/1000000);
          }
